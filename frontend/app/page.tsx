    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          ...
          XTSLA: { price: 4.2, lastUpdated: new Date() },
        })
      }, 500)
    })
  },
...
export default function Component() {
  ...
  const [swapId] = useState("SW-" + Math.random().toString(36).substr(2, 9).toUpperCase())
  const [btcPrice, setBtcPrice] = useState(0)
  const [xtslaPrice, setXtslaPrice] = useState(0)
  const [priceLoading, setPriceLoading] = useState(true)
  const [lastPriceUpdate, setLastPriceUpdate] = useState(new Date())
  const [btcPriceData, setBtcPriceData] = useState(null)
  const [xtslaPriceData, setXtslaPriceData] = useState(null)
  ...
  const fetchRealTimePrices = async () => {
    ...
    try {
      ...
      const xtslaData = pricesData.XTSLA
      ...
      if (xtslaData) {
        setXtslaPrice(xtslaData.price)
        setXtslaPriceData(xtslaData)
      }

      setLastPriceUpdate(new Date())

      // Update offers with new prices
      if (btcData && xtslaData) {
        updateOffersWithRealPrices(btcData.price, xtslaData.price)
      }
    } catch (error) {
  ...
  const updateOffersWithRealPrices = (btcPrice: number, xtslaPrice: number) => {
    const baseRate = xtslaPrice / btcPrice
    const updatedOffers = mockOffers.map((offer, index) => ({
      ...offer,
      rate: baseRate * (0.95 + index * 0.02), // Slight variations between MMs
      realTimeRate: true,
    }))
  ...
  const refreshOffers = async () => {
    ...
    // Fetch fresh prices and update offers
  ...
  const simulateSwapProgress = async () => {
    for (let i = 0; i < swapSteps.length; i++) {
      ...
      setSwapProgress((prev) =>
        prev.map((step, index) => ({
          ...step,
          status: index <= i ? "completed" : index === i + 1 ? "current" : "pending",
        })),
      )
    }
            ...
            <div className="hidden lg:block">
              <ChainlinkPriceTicker
                onPriceUpdate={(pricesData) => {
                  ...
                  if (pricesData.XTSLA) setXtslaPrice(pricesData.XTSLA.price)
                  setLastPriceUpdate(new Date())
                }}
              />
            </div>
              ...
              <Card className="bg-white/10 backdrop-blur-md border-white/20 mb-8">
                ...
                <CardContent className="space-y-6">
                    ...
                    {btcAmount && selectedOffer && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-white">
                          1 BTC = {(((xtslaPrice / btcPrice) * btcPrice) / xtslaPrice).toFixed(4)} XTSLA
                        </span>
                      </div>
                    )}
                  </div>
                              ...
                              <CardContent className="p-4">
                                ...
                                <div className="space-y-2 text-sm">
                                  <div className="flex justify-between">
                                    ...
                                    <span className="text-white font-medium">{offer.rate.toFixed(6)} XTSLA/BTC</span>
                                  </div>
                                  ...
                                  {btcAmount && (
                                    <div className="flex justify-between">
                                      ...
                                      <span className="text-green-400 font-medium">
                                        {calculateOutput(btcAmount, offer.rate)} XTSLA
                                      </span>
                                    </div>
                                  )}

                                  {btcAmount && xtslaPrice > 0 && (
                                    <div className="flex justify-between">
                                      <span className="text-gray-400">USD Value:</span>
                                      <span className="text-blue-400 font-medium">
                                        $
                                        {(
                                          Number.parseFloat(calculateOutput(btcAmount, offer.rate)) * xtslaPrice
                                        ).toFixed(2)}
                                      </span>
                                    </div>
                                  )}
                  ...
                  <Button
                    ...
                    disabled={!btcAmount || !selectedOffer || Number.parseFloat(btcAmount) <= 0}
                    className="w-full h-12 bg-gradient-to-r from-orange-500 to-purple-600 hover:from-orange-600 hover:to-purple-700 text-white font-semibold text-lg"
                  >
                    <ArrowRight className="h-5 w-5 mr-2" />
                    Swap {btcAmount || "0"} BTC →{" "}
                    {selectedOffer ? calculateOutput(btcAmount || "0", selectedOffer.rate) : "0"} XTSLA
                  </Button>
                </CardContent>
              </Card>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
              <Card className="bg-white/10 backdrop-blur-md border-white/20">
                <CardHeader>
                  ...
                  <p className="text-gray-400">Swap ID: {swapId}</p>
                </CardHeader>
                <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-green-400 font-medium">
                          {selectedOffer ? calculateOutput(btcAmount, selectedOffer.rate) : "0"} XTSLA
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-400">Swap ID</p>
                        <p className="text-white font-mono text-xs">{swapId}</p>
                      </div>
                      <div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-green-400 font-medium text-lg">
                        {calculateOutput(btcAmount, selectedOffer.rate)} XTSLA
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-white">{selectedOffer.rate} XTSLA/BTC</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Fee:</span>
