...
export const PRICE_FEEDS = {
  ...
  TSLA_USD: "0x1ceDaaB50936881B3e449e47e40A2cDAF5576A4a", // TSLA/USD (for RWA simulation)
} as const
...
export class ChainlinkPriceService {
  ...
  private priceCache: Map<string, { data: PriceData; cacheTime: number }> = new Map()
  ...
  async getPriceFromContract(contractAddress: string): Promise<PriceData | null> {
    try {
      const contract = new ethers.Contract(contractAddress, PRICE_FEED_ABI, this.provider)

      // Get latest round data and decimals in parallel
      let roundData: [bigint, bigint, bigint, bigint, bigint] | undefined = undefined
      ...
      try {
        ...
        if (err.code === "CALL_EXCEPTION") {
          const [legacyAnswer, legacyTimestamp, legacyDecimals] = await Promise.all([
          ...
          if (legacyAnswer && legacyTimestamp) {
            roundData = [0n, BigInt(legacyAnswer), BigInt(legacyTimestamp), BigInt(legacyTimestamp), 0n]
            decimals = Number(legacyDecimals ?? 8)
            description = await contract.description().catch(() => "Legacy Feed")
          } else {
      ...
      console.error(`Error reading from Chainlink contract ${contractAddress}:`, error)
      return null
    }
  }
    ...
    try {
      ...
      switch (symbol) {
        ...
        case "ETH":
          ...
          break
        case "XTSLA":
          // For RWA tokens, we'll use TSLA feed and apply tokenization ratio
          const tslaData = await this.getPriceFromContract(PRICE_FEEDS.TSLA_USD)
          if (tslaData) {
            priceData = {
              ...tslaData,
              price: tslaData.price * 0.01, // 1 XTSLA = 0.01 TSLA shares
              description: "XTSLA / USD (Tokenized Tesla)",
            }
          }
          break
        default:
      ...
      if (priceData) {
        this.priceCache.set(cacheKey, { data: priceData, cacheTime: Date.now() })
      }
  ...
  async getAllPrices(): Promise<Record<string, PriceData | null>> {
    const [btcData, xtslaData] = await Promise.all([this.getPrice("BTC"), this.getPrice("XTSLA")])
  ...
  subscribeToRealTimeUpdates(callback: (prices: Record<string, PriceData | null>) => void): () => void {
  ...
  async getNetworkStatus(): Promise<{ blockNumber: number; gasPrice: string }> {
    try {
      const [blockNumber, gasPrice] = await Promise.all([this.provider.getBlockNumber(), this.provider.getFeeData()])
