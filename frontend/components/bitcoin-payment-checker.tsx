    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data.hasPayments && data.data.latestTransaction) {
          ...
          setPaymentStatus(
            `Payment detected: ${data.data.latestTransaction.amount} BTC`
          );
        } else {
    ...
    <div className="p-4 bg-white/10 rounded-lg">
      ...
      <div className="space-y-2">
        <p className="text-sm text-gray-400">
          Address: {walletAddress.substring(0, 16)}...
          {walletAddress.substring(walletAddress.length - 8)}
        </p>

        <p className={`text-sm ${getStatusColor()}`}>Status: {paymentStatus}</p>
        ...
        {lastPayment && (
          <div className="mt-4 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
            <h4 className="text-sm font-semibold text-green-400 mb-2">
              Latest Payment{" "}
              {lastPayment.confirmations === "confirmed" ? "✅" : "⏰"}
            </h4>
            <div className="space-y-1 text-xs text-gray-300">
              <div>Amount: {lastPayment.amount} BTC</div>
              <div>Status: {lastPayment.confirmations}</div>
              <div>
                TXID: {lastPayment.txid.substring(0, 16)}...
                {lastPayment.txid.substring(lastPayment.txid.length - 8)}
              </div>
        ...
        <div className="text-xs text-gray-500">
          Auto-refreshing every {pollInterval / 1000}s • Min amount: {minAmount}{" "}
          BTC
        </div>
      </div>
    </div>
