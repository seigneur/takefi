...
import { Bitcoin, TrendingUp, TrendingDown, RefreshCw, AlertTriangle, CheckCircle, Globe } from "lucide-react"
...
export function ChainlinkPriceTicker({ onPriceUpdate }: ChainlinkPriceTickerProps) {
  ...
  const PriceItem = ({
    ...
    return (
      <motion.div
        initial={{ scale: 1 }}
        animate={{ scale: priceData.price !== previousPrices[symbol] ? [1, 1.05, 1] : 1 }}
        ...
        <div className="text-sm">
          <div className="flex items-center space-x-2">
            ...
            {!loading && (
              <Badge
                variant="outline"
                className={`text-xs ${isStale ? "bg-yellow-500/20 text-yellow-400 border-yellow-500/30" : "bg-green-500/20 text-green-400 border-green-500/30"}`}
              >
                {isStale ? (
                  <>
          ...
          {!loading && (
            <div className="text-xs text-gray-500 mt-1">
              ...
              {staleness < 300000 && ( // Less than 5 minutes
                <>
                  <span className="mx-2">•</span>
                  <span className="text-green-400">{Math.round(staleness / 1000)}s ago</span>
                </>
              )}
            </div>
          )}
