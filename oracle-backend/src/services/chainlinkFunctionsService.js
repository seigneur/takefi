...
class ChainlinkFunctionsService {
  constructor() {
    ...
    this.networkRpcUrl =
      process.env.NETWORK_RPC_URL || "https://eth-sepolia.public.blastapi.io";
  ...
  async createRequestAndReadResult(awsSecretARN, ethTxHash) {
    try {
      if (!this.enabled) {
        throw new Error(
          "Chainlink Functions service is disabled due to missing configuration."
        );
      }
      ...
      if (
        !subInfo.consumers
          .map((c) => c.toLowerCase())
          .includes(this.consumerContractAddress.toLowerCase())
      ) {
        throw new Error(
          `Consumer contract ${this.consumerContractAddress} has not been added to subscription ${this.subscriptionId}`
        );
      }
      ...
      const gasPriceWei = BigInt(
        Math.ceil(ethers.utils.formatUnits(gasPrice, "wei").toString())
      );
      ...
      const estimatedCostLink = ethers.utils.formatUnits(
        estimatedCostJuels,
        18
      );
      ...
      const requestConfig = {
        source: fs
          .readFileSync(
            path.join(
              __dirname,
              "../../../cow-mm-server/src/preimage-retrieval.js"
            )
          )
          .toString(),
      ...
      logger.info(
        `Sending request to Chainlink Functions with transaction hash: ${requestTx.hash}`
      );
      const requestTxReceipt = await requestTx.wait(1);
      logger.info(
        `Confirmed transaction hash ${requestTxReceipt.transactionHash}`
      );
      ...
      const {
        ...
        errorString,
        fulfillmentCode,
      } = await responseListener.listenForResponseFromTransaction(
        requestTx.hash,
        undefined,
        NUM_CONFIRMATIONS,
        undefined
      ...
      switch (fulfillmentCode) {
        case FulfillmentCode.FULFILLED:
          if (errorString.length > 0) {
            throw new Error(
              `Request ${requestId} fulfilled with error: ${errorString}`
            );
          } else if (responseBytesHexstring == "0x") {
            throw new Error(
              `Request ${requestId} fulfilled with empty response data`
            );
          } else {
        ...
        default:
          throw new Error(
            `Request ${requestId} failed with fulfillment code: ${fulfillmentCode}`
          );
      }
